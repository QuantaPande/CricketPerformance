...
train_x_ = pd.read_csv('train-india.csv', header = None)
...
train_x[:, 0:10] = (train_x[:, 0:10] - np.mean(train_x[:, 0:10], axis = 0))/(np.std(train_x[:, 0:10], axis = 0))

test_x_ = pd.read_csv('test-india.csv', header = None)
...
with open('train_india_x.csv', 'w', newline = '') as f:
    writer = csv.writer(f)
    writer.writerows(train_x_batches)

with open('train_india_y.csv', 'w', newline = '') as f:
    writer = csv.writer(f)
...
with open('test_india_x.csv', 'w', newline = '') as f:
    writer = csv.writer(f)
    writer.writerows(test_x_batches)

with open('test_india_y.csv', 'w', newline = '') as f:
    writer = csv.writer(f)
