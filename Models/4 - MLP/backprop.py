...
class backprop():
    def __init__(self, input, target, n_hidden, hidden, max_iter = 10000, eta = 1, f_type = "R", b_type = "R", b = 100, disp_step = 10):
        ...
        with tf.device("/gpu:0"):
            self.__w = [tf.Variable(tf.random_normal([input.shape[1], hidden]), name = 'weight_in')]
            for i in range(n_hidden - 1):
                self.__w.append(tf.Variable(tf.random_normal([hidden, hidden]), name = "weights_h_" + str(i)))
            self.__w.append(tf.Variable(tf.random_normal([hidden, target.shape[1]]), name = "weight_out"))
            self.__b_f = [tf.Variable(tf.random_normal([1, hidden]), name = "bias_f_in")]
            for i in range(n_hidden - 1):
                self.__b_f.append(tf.Variable(tf.random_normal([1, hidden]), name = "bias_f_h_" + str(i)))
            self.__b_f.append(tf.Variable(tf.random_normal([1, target.shape[1]]), name = "bias_f_out"))
            self.__b_b = [tf.Variable(tf.random_normal([1, hidden]), name = "bias_b_in")]
            for i in range(n_hidden - 1):
                self.__b_b.append(tf.Variable(tf.random_normal([1, hidden]), name = "bias_b_h_" + str(i)))
